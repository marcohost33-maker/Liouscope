...
- `bootstrap_B` controls the parametric bootstrap. The report records whether\n  its interval is BC or BCa; the default 80-point path is BC because no\n  acceleration jackknife is run above 60 points (issue #116). More resamples\n  reduce Monte-Carlo noise but do not guarantee a narrower interval.
...
print(r.relaxation.bca_ci_beta) # legacy tuple name: 95% interval (lo, hi)\nprint(r.relaxation.interval_method) # "BC", "BCa", or "unavailable"
