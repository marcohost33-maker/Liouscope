...
def compute_uncertainty_layer(
    ...
    *,
    ...
    U1 (solver) defaults to a conservative ``1e-10`` unless supplied by the
    caller from an ODE-tolerance sweep.
    U2 (size) is supplied externally (e.g. from finite-size scaling).
    """
    ...
    if np.isfinite(lo) and np.isfinite(hi):
        ...
        u0 = float("nan")
    u1 = float(solver_residual) if solver_residual is not None else 1.0e-10
