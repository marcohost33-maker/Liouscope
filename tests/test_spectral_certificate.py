...
def test_ambiguity_split_keeps_margin_on_both_sides() -> None:
    ...
    ``benchmarks/calibration/zero_mode_calibration.json``) and swept over ten
    seeds in ``benchmarks/calibrate_zero_mode_seed_sweep.py`` (artefact:
    ``benchmarks/calibration/zero_mode_seed_sweep.json``), the largest genuine
    in-band |lambda| over the healthy population reaches 4.71 * eps*||L||,
    while the marginal unresolved case sits at 4.87. The nearest pair is thus a
    factor 1.03 apart -- not the ~2x this docstring claimed while the sample
    behind it was uncommitted, and not the 1.15 that one seed suggested.
