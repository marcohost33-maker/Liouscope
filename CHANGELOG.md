...
### Changed
- **CAR(1) model selection now fits and scores one stationary likelihood consistently (PR #127 statistical review).** On non-uniform grids, `estimate_car1_theta` previously maximized a conditional transition likelihood that omitted the first residual, while `fit_gls_ar1` reported a stationary exact joint likelihood that included it. The CAR(1) rate is now optimized against that same stationary joint likelihood, matching the stationary process generated by `car1_resample`. AICc also counts both CAR(1) nuisance parameters estimated from each candidate fit: `theta` and the profiled stationary Gaussian variance. This is a METHODOLOGY change and can move AICc ordering, the selected M0..M3b model, reported decay rates, and downstream A-class on non-uniform grids; archived analyses whose model selection matters should be re-run. The historical uniform-grid AR(1) parameter-count convention is unchanged in this PR, and the run-manifest schema is unchanged.
